...
function Chord(opts, bootstrap) {
	this.opts = Object.assign({
		...
		peerOptions: { },
		nodeOptions: { subscriptionExpiration: 1000 * 60 * 1440 * 365 },
		hubOptions: { },
...
Chord.prototype.getWRTCString = function(onsign = text => {}) {
...
Chord.prototype.makeWebRTCconnection = function(offer,
	onsign = function(){}, onconn = function(){}, onGetId = function(){}) {
	var initiator = !offer, trickle = false,
		peer = new SimplePeer({ initiator, trickle });

	if(initiator) {
		peer.getAnswer = function (answer) {
			this.signal(JSON.parse(answer));
		}
	} else {
	...
	peer.once('signal', function (data) {
		console.log("SIGN");
		this.connText = JSON.stringify(data);
		onsign(this);
	});

	peer.parent = this;
	
	peer.getPeerId = function (data) {
		var obj = JSON.parse(data);

		if(obj.type == "id") {
			this.parent.hub.add(obj.id, this);
			console.log(obj.id);

			co(function *() {yield *this.joinOrRetry(obj.id)}.bind(this.parent));
			setTimeout(onGetId, 0, obj.id);
		} else {
			this.once('data', this.getPeerId);
		}
	};

	peer.once('data', peer.getPeerId);

	peer.once('connect', function () {
		console.log("CONNECT");
		this.send(JSON.stringify({type: "id", id: this.parent.id}));
		onconn(this);
	});
...
Chord.prototype.get_groupPubKeys = function() {
	return co(this.node.get_groupPubKeys())
}
