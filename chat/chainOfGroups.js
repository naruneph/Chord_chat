function ChainOfGroups ($_, context){
    ...
    this.findChains = function(groups){
        ...
        if(grList1.length && grList2.length){
            ...
            if(intersectionList.length){ //in the same room
    ...
    function path_1(rvList, gr, grList2, resList, groups, output){
        ...
        for(var i = 0; i < new_grList1.length; i++){

            if(intersection([new_grList1[i]], grList2).length){

                var tmp = [].concat(local_resList);
                tmp.push(new_grList1[i]);
                output.push(tmp);

            } else {

                if(local_resList.length < 6){
